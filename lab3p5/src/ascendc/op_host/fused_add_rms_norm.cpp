...
static ge::graphStatus TilingFunc(gert::TilingContext* context) {
    ...
    // --- Multi-core: split rows across the available AIV cores. ---
    auto ascendcPlatform = platform_ascendc::PlatformAscendC(context->GetPlatformInfo());
    int32_t coreNumAiv = static_cast<int32_t>(ascendcPlatform.GetCoreNumAiv());
    int32_t blockDim = coreNumAiv;
    if (B < blockDim) blockDim = B;
